...
class DataManager(object):
    ...
    def __init__(self, dataset_name='imdb',
        ...
        elif self.dataset_name == 'affectNet':
            self.dataset_path = '../datasets/affectNet/training_data.csv'
        else:
            raise Exception(
    ...
    def get_data(self):
        if self.dataset_name == 'imdb':
            ...
            ground_truth_data = self._loadAffectNet()
    ...
    def _loadAffectNet(self):
        ...
        with open(self.dataset_path) as csvfile:
            ...
            for i,row in enumerate(readCSV):
                ...
                filename = os.path.join('../datasets/affectNet/',row[1])
                ...
                gray_crop = gray[y1:y1+height, x1:x1+width]
                gray_image = cv2.resize(gray_crop.astype('uint8'), self.image_size )

                faces.append(gray_image.astype('float32'))
                emotions.append( int(row[6]) )
